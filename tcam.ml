...
(*    Array.print (fun oc {mask=m;bits=b} -> fprintf oc "m%a b%a" Int.xprint m Int.xprint b) oc a; *)
  ...
  let of_nrange x y = try of_nrange x y with Failure _ ->
    eprintf "Failure converting range: %a (widths: %a)\n%!" (List.print (Tuple2.print Int.print Int.print)) y (List.print Int.print) x;
    failwith "Exiting"
...
type 'a t_rul = (Entry.t, 'a) rul
type 'a tcam_rs = 'a t_rul Vect.t
...
let decs t = Vect.fold (fun acc r -> Set.add r.dec acc) Set.empty t
